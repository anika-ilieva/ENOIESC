...
from model_thesis_pure_conj_term import Multi2OIE_pure_conj_term
...
def get_models(bert_config,
               ...
               device=None):
    if conj_mode:
        return Multi2OIE_pure_conj_term(
            bert_config=bert_config,
            mh_dropout=mh_dropout,
            pred_clf_dropout=pred_clf_dropout,
            arg_clf_dropout=arg_clf_dropout,
            conjunction_clf_dropout=conjunction_clf_dropout,
            all_conj_expr_clf_dropout=all_conj_expr_clf_dropout,
            all_conj_term_clf_dropout=all_conj_term_clf_dropout,
            n_arg_heads=n_arg_heads,
            n_arg_layers=n_arg_layers,
            n_all_conj_expr_heads=n_all_conj_expr_heads,
            n_all_conj_expr_layers=n_all_conj_expr_layers,
            n_all_conj_term_heads=n_all_conj_term_heads,
            n_all_conj_term_layers=n_all_conj_term_layers,
            pos_emb_dim=pos_emb_dim,
            pred_n_labels=pred_n_labels,
            arg_n_labels=arg_n_labels,
            conjunction_n_labels=conjunction_n_labels,
            all_conj_expr_n_labels=all_conj_expr_n_labels,
            all_conj_term_n_labels=all_conj_term_n_labels).to(device)
    elif not use_lstm:
        return Multi2OIE(
            bert_config=bert_config,
            mh_dropout=mh_dropout,
...
class SummaryManager:
    def __init__(self, config, conj_mode=False):
        self.config = config
        self.save_config()
        if conj_mode:
            columns = ['epoch', 'train_predicate_loss',
                'train_argument_loss', 'train_conjunction_loss', 'train_conj_expr_loss', 'train_conj_term_loss']
        else:
